...
export const metadata = {
  title: sitePageTitle('Наадмын түүх'),
}
...
export default async function HistoryPage() {
  ...
  const history: any[] = [...baseEditions, ...addedEditions]
    .sort((a, b) => ORDER.indexOf(b.num) - ORDER.indexOf(a.num))
  const hasV = history.some(ed => ed.num === 'V')
  const newestNum = history[0]?.num
    ...
    <>
      ...
      <div style={{ background: 'var(--bg)', paddingBottom: 80 }}>
        <div className="wrap-wide" style={{ paddingTop: 52 }}>
          ...
          {/* Одоогийн наадам — V түүхэнд бүртгэгдээгүй үед дээрээ харуулна */}
          {!hasV && (
          <div style={{
            ...
            <div key={ed.num} style={{
              ...
              <div style={{ textAlign: 'center', paddingTop: 4 }}>
                <div style={{
                  ...
                  background: ed.num === newestNum ? 'var(--gold)' : 'var(--ink)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: 'Oswald, sans-serif', fontSize: 22, fontWeight: 900,
                  color: ed.num === newestNum ? 'var(--ink)' : 'var(--paper)',
                  margin: '0 auto',
                }}>
                  {ed.num}
                </div>
